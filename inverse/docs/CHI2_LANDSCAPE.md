...
| 建议 workers | 8 | 4 |
| 无噪声输出 | `../runs/inverse/chi2_landscape/p80/` | `../runs/inverse/chi2_landscape/p300/` |
| 有噪声输出 | `.../p80_noisy/`（`--noisy` 自动改名） | `.../p300_noisy/` |
...
`--noisy` 写到 `p80_noisy/` / `p300_noisy/`，不会盖掉无噪声结果。中断后重跑会读**该目录**里已有的 `chi2_cd_depth.npz`。不要两个终端写同一个输出目录。
