#!/usr/bin/env bash
# S4 total field -> reflected (TE incident subtracted) -> spatial + k-space plots.
# Invoked by run_s4_nearfield5.sh.
...
# S4 reflected-field conventions (see README.txt)
S4_PHASE_SIGN=${S4_PHASE_SIGN:-plus}
S4_TE_SIGN=${S4_TE_SIGN:-1}
S4_CARRIER_GAUGE=${S4_CARRIER_GAUGE:-none}
S4_Z_TERM_SIGN=${S4_Z_TERM_SIGN:-plus}
# minus = exp(-i kx x) in plot_kspace; none when carrier already removed
...
for req in \
  ...
  "${NEAR_DIR}/plot_s4_nearfield_viz.py" \
  "${NEAR_DIR}/s4_total_to_reflected.py"; do
...
echo "=== S4 near-field: reflected workflow ==="
echo "S4: ${S4_BIN}"
...
echo "k-space demod: ${KSPACE_DEMOD_SIGN} (carrier_gauge=remove forces none)"
